...
const unsigned long WEBSOCKET_COMMAND_INTERVAL = 200;
...
void setup() {
  ...
  if (!motorMutex || !statusMutex || !sensorMutex || !motorCommandQueue) {
  ...
  // Cấu hình PWM
  if (!ledcAttach(ENA, 500, 8)) {
    Serial.println("ERROR: Failed to attach ENA PWM");
    motorEnabled = false;
  }
  if (!ledcAttach(ENB, 500, 8)) {
    ...
    case WStype_CONNECTED:
      {
        ...
        webSocket.sendTXT(num, "CAMERA:http://" + WiFi.localIP().toString() + ":82/stream");
...
void handleSocketServer() {
  if (!currentClient || !currentClient.connected()) {
    ...
    if (currentClient) {
      ...
      sendSocketMessage("CAMERA:http://" + WiFi.localIP().toString() + ":82/stream");
      lastCommandTime = millis();
    }
  }
...
void tien() {
  ...
  // Đặt PWM trực tiếp thay vì vòng lặp blocking
